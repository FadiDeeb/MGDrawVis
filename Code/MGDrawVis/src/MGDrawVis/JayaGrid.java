...
public class JayaGrid extends Drawer 
{
    ...
    public JayaGrid(DrawingStageActions draw, Graph graph, int phase, boolean animatedEnabled)
    {
    ...
    private void generateJayaCandidates(Graph graph)
    {
        ...
        for (int k = 0; k < populationSize; k+=5)
        {
           upperLeftCandidate(k);
           upperRightCandidate(k+1);
           lowerLeftCandidate(k+2);
           lowerRightCandidate(k+3);
           randomLocationCandidate(k+4);
        }
    }
    public void upperLeftCandidate(int k)
    {
        ...
        for (int i=0; i<jayaCandidates.get(k).points.size(); i++)
        {
            int x = (int) ((Math.random() * ((sx+width)/2)));      
            int y = (int) ((Math.random() * ((sy+height)/2)));
            Vertex v = new Vertex(x,y,jayaCandidates.get(k).nodeSize);
    ...
    public void upperRightCandidate(int k)
    {
        Bounds bounds = DrawingStage.screen.getBoundsInLocal();
        Bounds screenBounds = DrawingStage.screen.localToScene(bounds);
        int sx = (int) DrawingStage.screen.getTop().getLayoutX();
        ...
        for (int i=0; i<jayaCandidates.get(k).points.size(); i++)
        {
            int x = (int) (((sx+width)/2)+((Math.random() * ((sx+width)/2))));      
            int y = (int) ((Math.random() * ((sy+height)/2)));
            Vertex v = new Vertex(x,y,jayaCandidates.get(k).nodeSize);
            jayaCandidates.get(k).updateVertex(i, v);
        }
    ...
    public void lowerLeftCandidate(int k)
    {
        Bounds bounds = DrawingStage.screen.getBoundsInLocal();
        Bounds screenBounds = DrawingStage.screen.localToScene(bounds);
        int sx = (int) DrawingStage.screen.getTop().getLayoutX();
        ...
        {
            int x = (int) ((Math.random() * ((sx+width)/2)));      
            int y = (int) (((sy+height)/2)+((Math.random() * ((sy+height)/2))));
            Vertex v = new Vertex(x,y,jayaCandidates.get(k).nodeSize);
            jayaCandidates.get(k).updateVertex(i, v);
        }
        updateResults(jayaCandidates.get(k));
    ...
    public void lowerRightCandidate(int k)
    {
        Bounds bounds = DrawingStage.screen.getBoundsInLocal();
        Bounds screenBounds = DrawingStage.screen.localToScene(bounds);
        int sx = (int) DrawingStage.screen.getTop().getLayoutX();
    ...
    public void randomLocationCandidate(int k)
    {
        Bounds bounds = DrawingStage.screen.getBoundsInLocal();
        Bounds screenBounds = DrawingStage.screen.localToScene(bounds);
        int sx = (int) DrawingStage.screen.getTop().getLayoutX();
